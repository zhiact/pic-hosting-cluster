...
const fetchPromises = requests.map(request => {
  const { url, headers, source, repo } = request;

  return fetch(new Request(url, {
    method: 'GET',
    headers: headers
  })).then(async response => {
    // 确保 processResponse 仅在它存在的情况下调用
    if (from === 'where' && 'processResponse' in request && typeof request.processResponse === 'function') {
      try {
        const result = await request.processResponse(response);
